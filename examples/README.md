...
- An opt-in leased snapshot source for user-operated point-in-time backup jobs (within or outside of Kubernetes)

Assuming you've deployed `sg-cosi` as per the main README and got the details right, you can try either or both of these.

The leased workflow uses `examples/snapshotlease.yaml` and requires `snapshotLease.enabled=true` in the Helm values. See `docs/snapshotlease.md` for creation, renewal, and cleanup semantics.

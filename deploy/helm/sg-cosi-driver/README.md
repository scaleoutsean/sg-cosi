...
| `snapshotLease.enabled` | bool | `false` | Deploy the alpha SnapshotLease controller |
| `snapshotLease.replicas` | int | `1` | SnapshotLease controller replicas; leader election permits more than one |
| `snapshotLease.maxLifetime` | string | `168h` | Maximum lifetime persisted for each new lease |
| `snapshotLease.credentialGrace` | string | `1h` | AccessKey expiry grace after the hard lease deadline |
| `snapshotLease.resyncInterval` | string | `1h` | Safety interval between active lease reconciliations |
| `snapshotLease.leaderElection` | bool | `true` | Enable controller leader election |
| `snapshotLease.resources` | object | see `values.yaml` | Resource requests and limits for the controller |

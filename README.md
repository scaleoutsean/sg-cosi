...
| Leased Snapshot Sources | alpha, opt-in | Creates a snapshot and AccessKey for a bounded, manually renewable lifetime, then performs ordered cleanup. |
...
For backup and long-running analysis jobs, the optional [`SnapshotLease`](docs/snapshotlease.md) controller manages snapshot, credentials, renewal, and eventual cleanup. It deliberately does not run data transfers or manage backup destinations.
